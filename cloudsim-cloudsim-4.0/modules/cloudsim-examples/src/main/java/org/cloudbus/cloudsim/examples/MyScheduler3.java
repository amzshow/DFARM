...
public class MyScheduler3 {
	// Used for latest paper, removed sorting in favour of find min
	// Leads to better time complexity as sorting has O(nLogn) and min has O(n)
	...
	private static int SORT_TYPE = 3;
	// 0 - No Sort
	// 1 - Deadline
	// 2 - Deadline / Length
	// 3 - Length / Deadline

	private static int ADJUST_DEEP_SEARCH = 0;
	...
	// 0 is none, 0.5 is halfs while 1 is all.
	...
	// 1-30 SYN
	// 100-400 GOOGLE
	private static double DEADLINE_MIN = 20.0;
	private static double DEADLINE_MAX = 40.0;
	...
	private static List<EVm> createVMAdvance(int userId, int vms, int mipslow, int mipshigh) {
		...
		for (int i = 0; i < vms; i++) {
			int mips = MyScheduler3.genRandom(mipslow, mipshigh);
...
//				}
				...
				int selectedCloudletPos = findMin(remainingCloudlet);
				
				ECloudlet selectedCloudlet = remainingCloudlet.remove(selectedCloudletPos);
...
//					}
	...
	private static int findMin(List<ECloudlet> remainingCloudlet) {
		int min_i = 0;
		ECloudlet min = null;

		for (int i = 0; i < remainingCloudlet.size(); i++) {
			ECloudlet cloudlet = remainingCloudlet.get(i);
			if (Objects.isNull(min) || (cloudlet.getDeadlineTime() + 1) / (cloudlet.getCloudletLength() + 1) < (min.getDeadlineTime() + 1) / (min.getCloudletLength() + 1) ) {
				min = cloudlet;
				min_i = i;
			}
		}
