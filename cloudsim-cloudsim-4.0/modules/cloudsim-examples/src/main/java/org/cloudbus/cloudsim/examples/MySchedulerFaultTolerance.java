...
public class MySchedulerFaultTolerance {
	...
	private static int ALLOW_REJECTS_TO_RUN = 1;
	// 0 - Will not let rejected task run
	// 1 - Will let rejected task run
	
	private static int ALLOW_FAULT_TOLERANCE = 1;
	// 0 - Task will run with no backup
	// 1 - Let the task replicate or resubmit
	
	private static double PREACQUIRED_VM = 1;
	// How many VM should be availabxle from the start
	// 0 is none while 1 is all.
	
	// 0.0 - 1.0
	private static double REPL_RESUB_RATIO = 0;
	...
	private static int COUNT_REPLICATED = 0;
	private static int COUNT_RESUBMITTED = 0;
	private static int COUNT_KILLED = 0;
	...
	// VM ID that will fail
	public static int[] VM_ID_WILL_FAIL = {0};
	
	// RANDOMLY BREAK VM
	public static int BREAK_RANDOM_VM = 0;
	...
	private static List<EVm> createVMAdvance(int userId, int vms, int mipslow, int mipshigh) {
		...
		for (int i = 0; i < vms; i++) {
			int mips = MySchedulerFaultTolerance.genRandom(mipslow, mipshigh);
	...
	private static EVm checkAvailableVms(List<EVm> availableVmList, ECloudlet selectedCloudlet, boolean onlyWithinDeadline) {
		...
		for (EVm availableVm : availableVmList) {
			if(selectedCloudlet.isDuplicate() && availableVm.getHost().getId() == selectedCloudlet.getPreviousAssignedVmId()) continue;
	...
	private static ECloudlet checkAdjustPossible(ECloudlet cloudlet, List<ECloudlet> cloudletList, EVm vm,
		...
		for (int i = cloudletList.size() - 1; i >= 0; i--) {
			...
			if(cloudlet.isDuplicate() && vm.getHost().getId() == cloudlet.getPreviousAssignedVmId())
				continue;
	...
	private static boolean isReplicationRequired(ECloudlet cloudlet, List<Double> history, double REPL_RESUB_RATIO){
		// TODO Check wether replication is required
		
		if(history.size() == 0)
			return true;
		
		boolean replicationRequired = true;
		
		int index = (int) (REPL_RESUB_RATIO * (history.size() - 1));
		double val = sortFunction(cloudlet);
		
		if(val >= history.get(index))
	...
	private static ECloudlet checkAdjustPossibleV2(ECloudlet cloudlet, List<ECloudlet> cloudletList, Map<Integer, EVm> vmMap, Map<Integer, Double> vmCtMap) {
		...
		for(Integer vmId : vmMap.keySet()) {
			if(vmMap.get(vmId).getHost().getId() == cloudlet.getPreviousAssignedVmId())
				continue;
	...
	private static double sortFunction(ECloudlet cloudlet, int sort_type) {
		if(sort_type == 1)
			return cloudlet.getDeadlineTime() + 1;
		else if(sort_type == 2)
			return (cloudlet.getDeadlineTime() + 1) / (cloudlet.getCloudletLength() + 1);
		else if(sort_type == 3)
			return (cloudlet.getCloudletLength() + 1) / (cloudlet.getDeadlineTime() + 1);
		else
			return 0.0;
	}
	
	private static double sortFunction(ECloudlet cloudlet) {
		return sortFunction(cloudlet, SORT_TYPE);
	}
	...
	public static void run(int n) {
		...
		try {
			...
			for(EVm vm: vmList) {
				datacenter0.getVmAllocationPolicy().allocateHostForVm(vm);
			}
			
			// TODO Random fail
//			if(ALLOW_FAULT_TOLERANCE == 1 && BREAK_RANDOM_VM == 1)
//				VM_ID_WILL_FAIL[0] = genRandom(0, vmList.size() - 1);
			...
			int nextCloudletId = cloudletList.size();
			...
			for(ECloudlet cl: cloudletList) {
				history.add(sortFunction(cl));
				history = history.stream().sorted().collect(Collectors.toList());
			}
			...
			if (SORT_TYPE >= 1) {
				remainingCloudlet
					.sort((o1, o2) -> Double.compare(sortFunction(o1), sortFunction(o2)));
			}
...
//			}
			...
			while (remainingCloudlet.size() != 0) {
				...
				if(!selectedCloudlet.isDuplicate() && ALLOW_FAULT_TOLERANCE == 1)
					requiresReplication = isReplicationRequired(selectedCloudlet, history, REPL_RESUB_RATIO);

				for (int vmi = 0; vmi < vmctSortedId.size(); vmi++) {
					
					Integer vmId = vmctSortedId.get(vmi);
					// Deadline Check

					EVm vm = vmMap.get(vmId);
					
					if(selectedCloudlet.isDuplicate() && vm.getHost().getId() == selectedCloudlet.getPreviousAssignedVmId())
						continue;
					...
					if (newVmCt <= deadline) {
						foundVm = true;
						assignedVmId = vmId;
						if(!selectedCloudlet.isDuplicate())
				...
				if (!foundVm) {
					...
					if (!FIND_NEW_VM_WITHIN_DEADLINE && acquiredVm != null) {
						...
						if(vmctSortedId.size() > 0 && vmctMap.get(vmctSortedId.get(0)) + cloudletCt <= acquiredVmCt) {
							acquiredVm = null;
							assignedVmId = vmctSortedId.get(0);
							foundVm = true;
							if(!selectedCloudlet.isDuplicate())
...
//				if (!requiresReplication && foundVm && ALLOW_FAULT_TOLERANCE == 1) {
//					for(Integer vm_id_will_fail : VM_ID_WILL_FAIL) {
//						if(vm_id_will_fail.equals(assignedVmId)) {
//							assignedVmId = null;
//							foundVm = false;
...
//				}
				...
				if(foundVm && ALLOW_FAULT_TOLERANCE == 1 && !selectedCloudlet.isDuplicate()) {
					if(!requiresReplication) {
					 ...
					 if(requiresReplication) {
						...
						ECloudlet newCloudlet = createCloudlet(nextCloudletId, 0, selectedCloudlet.getCloudletLength());
						newCloudlet.setArrivalTime(selectedCloudlet.getArrivalTime());
						newCloudlet.setDeadlineTime(selectedCloudlet.getDeadlineTime());
						newCloudlet.setPreviousAssignedVmId(assignedVmId);
						newCloudlet.setDuplicate(true);
						List<ECloudlet> cll = new LinkedList<>();
						cll.add(newCloudlet);
						broker.submitCloudletList(cll);
						cloudletList.addAll(cll);
						broker.getCloudletList().addAll(cll);
						remainingCloudlet.addAll(0, cll);
						nextCloudletId = nextCloudletId + 1;
...
//				}	
//				
//				if(ALLOW_FAULT_TOLERANCE == 1 && !selectedCloudlet.isDuplicate()) {
//					if(requiresReplication) {
//						COUNT_REPLICATED = COUNT_REPLICATED + 1;
//						cloudletList.remove(selectedCloudlet);
//					} else
...
//					}
			...
			Integer acceptedSize = (int) (totalTasks - rejectionRate);
			Integer rejectedSize = rejectedCloudlet.size();
			...
			Double throughput = acceptedSize / makespan;
			Double arur = averageMakeSpan / makespan;
			
			if(makespan.isNaN()) makespan = 0.0;
			if(throughput.isNaN()) throughput = 0.0;
			if(arur.isNaN()) arur = 0.0;
			...
			System.out.println(String.format("Rejected: %d", rejectedSize));
			System.out.println(String.format("Accepted: %d", acceptedSize));
			...
			System.out.println(String.format("Replicated: %d", COUNT_REPLICATED));
			System.out.println(String.format("Resubmitted: %d", COUNT_RESUBMITTED));
			System.out.println(String.format("Killed: %d", COUNT_KILLED));
