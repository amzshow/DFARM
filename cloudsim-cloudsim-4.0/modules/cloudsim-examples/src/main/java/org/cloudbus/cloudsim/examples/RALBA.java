...
public class RALBA {
	...
	private static int SCHEDULER = 4;
	// 0 RS
	// 1 RR
	// 2 MCT
	// 3 RALBA
	...
	private static boolean SScheduler(List<Cloudlet> cloudletList, List<Vm> vmList, Map<Integer, Double> DEADLINES, Cloudlet selectedCloudlet, Vm vm, double deadline, double currentVmCt) {
		...
		for(int i = 0; i < sortedDeadline.size(); i++) {
			...
			newVmCt = currentVmCt + selectedExecTime;
			selectedNewCt = candidateCt - candidateExecTime + selectedExecTime;
			
			if(newVmCt < candidateDeadline && selectedNewCt < deadline) {
		...
		if(flag == 1) {
			Collections.swap(cloudletList, cloudletList.indexOf(candidate), cloudletList.indexOf(selectedCloudlet));
			return true;
		}
