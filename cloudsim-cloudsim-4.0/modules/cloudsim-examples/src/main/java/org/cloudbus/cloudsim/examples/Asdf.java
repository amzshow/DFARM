...
public class Asdf {
	...
	/** The cloudlet list. */
	private static List<ECloudlet> cloudletList;

	/** The vmlist. */
	private static List<EVm> vmList;
	...
	private static ECloudlet getSmallestCloudlet(HashMap<Integer,ECloudlet> remainingCloudlet) {
		ECloudlet smallest_cl = null;
		for(ECloudlet cl: remainingCloudlet.values()) {
			if(Objects.isNull(smallest_cl) || cl.getCloudletLength() < smallest_cl.getCloudletLength()) {
				smallest_cl = cl;
	...
	private static ECloudlet getLargestCloudlet(HashMap<Integer,ECloudlet> remainingCloudlet) {
		ECloudlet largest_cl = null;
		for(ECloudlet cl: remainingCloudlet.values()) {
			if(Objects.isNull(largest_cl) || cl.getCloudletLength() > largest_cl.getCloudletLength()) {
				largest_cl = cl;
	...
	private static HashMap<Integer, Double> getVmCrMap(List<EVm> vmlist) {
	...
	private static Vm getVMwithEFT(HashMap<Integer, Double> cloudlet_ct_per_vm, List<EVm> vmList) {
	...
	private static HashMap<Integer, Double> getCloudletCtPerVm(ECloudlet cl, List<EVm> vmList, HashMap<Integer, Double> vm_ct) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (SCHEDULER == 0) {
				...
				HashMap<Integer, ECloudlet> remainingCloudlet = new HashMap<>();
				...
				while (true) {
					...
					ECloudlet cl = getSmallestCloudlet(remainingCloudlet);
					...
					ECloudlet clToAssign = getLargestCloudletToAssignVM(vmshare.get(largest_vmshare_id),
							remainingCloudlet);
					...
					if (getVmCt(vmList, cloudletList, clToAssign).get(largest_vmshare_id) > clToAssign.getDeadlineTime()) {
				...
				while (remainingCloudlet.size() != 0) {
					...
					ECloudlet cl = getLargestCloudlet(remainingCloudlet);
					...
					if (getVmCt(vmList, cloudletList, cl).get(vmid) > cl.getDeadlineTime()) {
			...
			// RANDOM
				...
				for (int i = 0; i < cloudletList.size(); i++) {
					...
					ECloudlet cl = cloudletList.get(i);
					boolean rejected = false;
					if (getVmCt(vmList, cloudletList, cl).get(vmid) > cl.getDeadlineTime()) {
			...
			// Minimum Completion Time
				...
				for(int i = 0; i < cloudletList.size(); i++) {
					...
					ECloudlet cl = cloudletList.get(i);
					if (vmCt.get(vmId) > cl.getDeadlineTime()) {
						rejectedCloudlet.add(cl);
						rejected = true;
						if (ALLOW_REJECTS_TO_RUN == 0) {
							cloudletList.remove(cl);
